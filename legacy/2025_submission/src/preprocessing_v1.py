...
def _clean_datetime_text(series: pd.Series) -> pd.Series:
    ...
    s = s.str.replace(r"(\d{1,2}:\d{2})(?!:)", r"\1:00", regex=True)
    # collapse commas
...
def _to_uniform_utc_naive(series: pd.Series, assume_local_tz: str = _AUSTIN_TZ) -> pd.Series:
    ...
    dt1 = pd.to_datetime(raw, errors="coerce", utc=True, format="mixed")
    ...
    if m2.any():
        ...
        dt2 = (
            dt2.dt.tz_localize(assume_local_tz, nonexistent="shift_forward", ambiguous="infer")
               .dt.tz_convert("UTC")
               .dt.tz_localize(None)
        )
        out.loc[m2] = dt2
    ...
    if m3.any():
        ...
        dt3 = (
            dt3.dt.tz_localize(assume_local_tz, nonexistent="shift_forward", ambiguous="infer")
               .dt.tz_convert("UTC")
               .dt.tz_localize(None)
        )
        out.loc[m3] = dt3
